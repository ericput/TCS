...
class SpanBasedF1Measure:
    ...
    def __call__(self, predictions: List[List[str]], gold_labels: List[List[str]]):
        ...
        for i in range(size):
            predicted_spans = self.bio_tags_to_spans(predictions[i])
            if len(predictions[i]) != len(gold_labels[i]):
                print('The length of line %d are not equal: %d %d' % (i, len(predictions[i]), len(gold_labels[i])))

            gold_spans = self.bio_tags_to_spans(gold_labels[i])

            if self.ignore_type:
                predicted_spans = set([('Entity', item[1]) for item in predicted_spans])
                gold_spans = set([('Entity', item[1]) for item in gold_spans])
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 3 and sys.argv[3] != '0':
        measure = SpanBasedF1Measure(True)
    else:
